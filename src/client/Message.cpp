...
const stringVec &Message::getCmdParam() { return cmdParam_; }

/* the command being processed */
const std::string Message::getCmd() const { return cmdParam_.at(0); }

/* the unprocessed message */
const std::string Message::getMess() const { return message_; }

/* in the event where several messages separated by /r/n were sent, the leftover
 * part of the raw message */
const std::string Message::getLeft() const { return leftover_; }

size_t Message::getSize() const { return cmdParam_.size(); }
...
void Message::setMess(std::string buffer) { message_ = buffer; }
...
bool Message::lenIsValid(const Client &sender) {
  ...
  if (message_.size() > 512) {
    reply::send_(sender.getFd(), ERR_INPUTTOOLONG(sender.cliInfo.getNick()));
    return false;
  }
  return true;
}
...
bool Message::emptyBuff() { return message_.begin() == message_.end(); }

bool Message::isCap() { return message_.find("CAP") != std::string::npos; }
