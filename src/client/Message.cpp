...
std::string &
Message::operator[] (unsigned int i) {
	if (i >= cmdParam_.size ())
...
void
Message::setCmdParam (const stringVec &splitBuffer) {
...
void
Message::setMess (std::string buffer) {
	message_ = buffer;
}
...
bool
Message::hasPrefix (const std::string &cliPrefix) {
	if (message_.at (0) == ':') {
...
bool
Message::lenIsValid (const Client &sender) {
	if (message_.empty ())
		return false;
...
Message::evaluateTermination () const {
	if (message_.find ("\r\n") != std::string::npos)
		return 2;
	if (message_.find ("\n") != std::string::npos)
		return 1;
...
static bool
isConsecutiveSpace (char left, char right) {
	return (left == ' ' && right == ' ');
}
void
Message::trimSpaces () {
