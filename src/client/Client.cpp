...
void
Client::setCliEpoll (struct epoll_event epoll) {
	cliEpoll_.events = epoll.events;
	cliEpoll_.data.fd = epoll.data.fd;
}

void
Client::removeOneChan (std::string chanName) {
	for (stringVecIt it = joinedChans_.begin (); it != joinedChans_.end ();
		 ++it)
		if (*it == chanName) {
...
void
Client::addOneChan (std::string chanName) {
	stringVecItConst it;
