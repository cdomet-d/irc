...
void reply::log(e_level level, std::string message) {
  ...
  if (serv.logfile.is_open()) {
    ...
    case REPLY:
      ...
      break;
    case GOT:
      serv.logfile << "[" << timeStamp() << "] GOT " << message << std::flush;
      break;
    }
  }
}
...
void reply::log(e_level level, std::string message, std::string verbose) {
  ...
  if (serv.logfile.is_open()) {
    switch (level) {
    case (INFO):
      serv.logfile << "[" << timeStamp() << "] INFO  " << message << verbose
                   << std::flush;
      break;
    case (ERROR):
      serv.logfile << "[" << timeStamp() << "] ERROR " << message << verbose
                   << std::flush;
      break;
    case (DEBUG):
      serv.logfile << "[" << timeStamp() << "] DEBUG " << message << verbose
                   << std::flush;
      break;
    case (REPLY):
      serv.logfile << "[" << timeStamp() << "] REPLY " << message << verbose
                   << std::flush;
      break;
    case GOT:
      serv.logfile << "[" << timeStamp() << "] GOT " << message << std::flush;
