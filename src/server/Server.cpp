...
/*   Updated: 2025/04/07 17:28:13 by cdomet-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Server &
Server::GetServerInstance (int port, std::string password) {
...
void
Server::addChan (Channel *curChan) {
	channels_.insert (
		std::pair< std::string, Channel * > (curChan->getName (), curChan));
}

void
Server::removeChan (Channel *curChan) {
	channels_.erase (curChan->getName ());
}
void
Server::removeCli (Client *curCli) {
	removeNickFromUsedNicks (curCli->cliInfo.getNick ());
	clients_.erase (curCli->getFd ());
}

void
Server::addNickToUsedNicks (const std::string &newNick, int fd) {
	usedNicks_.insert (nickPair (newNick, fd));
}

void
Server::removeNickFromUsedNicks (const std::string &toRemove) {
...
const char *
Server::InitFailed::what () const throw () {
	std::cerr << "irc: ";
	return (errMessage);
}
...
const clientMap &
Server::getAllCli () const {
	return (clients_);
}
const channelMap &
Server::getAllChan () const {
	return (channels_);
}

const nickMap &
Server::getUsedNick () const {
	return (usedNicks_);
}

int
Server::getFdFromNick (const std::string &nick) const {
