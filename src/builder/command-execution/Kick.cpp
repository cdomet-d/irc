...
/*   Updated: 2025/03/18 15:19:03 by aljulien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void handleKick(CmdSpec &cmd)
{
	...
	for (size_t nbTarget = 0; nbTarget < cmd[target].getSize(); nbTarget++) {
		...
		if (cmd[message].getSize())
			sendMessageChannel(curChan->getCliInChan(), RPL_KICK(sender->cliInfo.getPrefix(), curChan->getName(), target_->cliInfo.getNick(), cmd[message][nbTarget]));
		else
			sendMessageChannel(curChan->getCliInChan(), RPL_KICK(sender->cliInfo.getPrefix(), curChan->getName(), target_->cliInfo.getNick(), ""));
