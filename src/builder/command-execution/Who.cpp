...
std::string
buildNickList (clientMap curMap, Client *sender, Channel &curChan) {
	std::string list;
	...
	for (clientMapIt it = curMap.begin (); it != curMap.end (); ++it) {
		if (it->first == sender->getFd ())
			continue;
		std::string prefix = "";
		if (curChan.getOpCli ().find (it->first)
...
who (CmdSpec &cmd) {
	...
	reply::send_ (sender->getFd (),
				  RPL_NAMREPLY (sender->cliInfo.getNick (), "=",
								curChan.getName (), nickList));
	reply::send_ (sender->getFd (), RPL_ENDOFNAMES (sender->cliInfo.getNick (),
													curChan.getName ()));
}