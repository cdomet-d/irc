...
void
executeO (std::string flag, std::string param, Channel &curChan) {
	...
	if (flag == "-o") {
		...
		reply::send_ (targetCli->getFd (),
					  RPL_CHANOPENOPE (targetCli->cliInfo.getNick (),
									   curChan.getName ()));
	}
}

void
executeI (std::string flag, std::string param, Channel &curChan) {
	(void)param;
...
void
executeT (std::string flag, std::string param, Channel &curChan) {
	(void)param;
...
void
executeK (std::string flag, std::string param, Channel &curChan) {
	if (flag == "+k") {
...
void
executeL (std::string flag, std::string param, Channel &curChan) {
	if (flag == "+l") {
		char *endptr;
		errno = 0;
...
int
findFlagLevel (std::string level) {
...
void
executeFlag (std::string flag, std::string param, Channel &curChan) {
...
Channel &
findCurChan (std::string chanName) {
...
void
sendModeMessages (std::string &first, std::string &second, Channel &curChan,
				  std::string nick) {
	std::string messages
...
void
buildNewModeString (CmdSpec &cmd, Channel &curChan, Client *sender) {
	std::string negMode = "-";
	std::string posMode = "+";
