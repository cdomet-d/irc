...
/*                               GETTERS                                      */
/* ************************************************************************** */
const std::string &CmdSpec::getName(void) const { return (name_); }

bool CmdSpec::getValid(void) const { return (valid_); }

void (*CmdSpec::getExecutor(void) const)(CmdSpec &cmd) { return (cmExecutor_); }

Client &CmdSpec::getSender(void) const { return (*sender_); }

const paramMap &CmdSpec::getParams(void) const { return (params_); }

int CmdSpec::getRegistrationStage() const { return (registrationStage_); }

const std::string CmdSpec::getSendNick() const {
  return sender_->cliInfo.getNick();
}

int CmdSpec::getSendFd() const { return sender_->getFd(); }

/* ************************************************************************** */
/*                               SETTERS                                      */
/* ************************************************************************** */
void CmdSpec::setSender(Client &sender) { sender_ = &sender; }
...
void CmdSpec::hasParamList(void) {
  for (size_t i = 0; i < params_.size(); i++) {
    ...
    if (innerParam.isList()) {
      try {
        innerParam.setParamList(buffer_manip::vectorSplit(innerParam[0], ','));
      } catch (const std::out_of_range &e) {
      };
    }
  }
...
CmdSpec *CmdSpec::CmdBuilder::build() {
  return (
      new CmdSpec(name_, registrationStage_, params_, checkers_, cmExecutor_));
}
