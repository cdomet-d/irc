...
bool
check::chan (CmdSpec &cmd, size_t idx) {
	std::string channel;
	...
	if (!check::chans_::onChan (channel, userChan)) {
		reply::send_ (cmd.getSendFd (),
					  ERR_NOTONCHANNEL (cmd.getSendNick (), channel));
		return (false);
	}
	return (true);
}
...
bool
check::chans_::onChan (std::string arg, const stringVec &arr) {
	return std::find (arr.begin (), arr.end (), arg) != arr.end ();
}

bool
check::chans_::isOp (CmdSpec &cmd, size_t idx) {
