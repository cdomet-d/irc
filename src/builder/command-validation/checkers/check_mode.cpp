...
e_mdeset
check::mode_::whichSet (const char &c) {
	switch (c) {
	case '+':
		return SET;
	case '-':
...
e_mdetype
check::mode_::typeIsValid (const char &c) {
	switch (c) {
	case 'i':
		return D;
	case 'k':
...
bool
check::mode_::flagIsValid (e_mdeset &set, e_mdetype &type,
 ...
 * argument*/
bool
check::mode_::formatArgs (CmdSpec &cmd) {
	e_mdetype type;
	e_mdeset set;
...
bool
check::mode (CmdSpec &cmd, size_t idx) {
	(void)idx;
