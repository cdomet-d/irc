...
bool
check::join (CmdSpec &cmd, size_t idx) {
...
bool
check::join_::assessRequest (Channel chan, CmdSpec &cmd, size_t idx) {
...
bool
check::join_::hasInvite (Channel &chan, Client &sender) {
...
bool
check::join_::validKey (Channel &chan, CmdParam &keys, size_t idx,
						Client &sender) {
...
bool
check::join_::chanHasRoom (Channel &chan, Client &sender) {
...
bool
check::join_::cliHasMaxChans (Channel &chan, Client &sender) {
...
bool
check::join_::syntaxIsValid (CmdSpec &cmd, size_t idx) {
	if (cmd[channel_][idx][0] != '#') {
		reply::send_ (cmd.getSendFd (), ERR_NOSUCHCHANNEL (cmd.getSendNick (),
														   cmd[channel_][idx]));
		return false;
	}
	return true;
