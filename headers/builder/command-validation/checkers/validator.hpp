...
const stringVec &getTargetChan(const std::string &target, const Server &serv);
template <typename MapType> bool exists(std::string arg, const MapType &map) {
...
bool flagIsValid(e_mdeset &set, e_mdetype &type, const std::string &flag,
                 const Client &cli);
